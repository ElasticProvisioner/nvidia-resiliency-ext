...
from .postprocessing import get_dataflow_stats, get_slack_stats, post_results
...
class AttributionService:
    ...
    def __init__(self, cfg: Settings):
        ...
        # Create library LogAnalyzer with dataflow posting callback
        self._analyzer = LogAnalyzer(
            config=analyzer_config,
            post_results_fn=post_results if cfg.DATAFLOW_INDEX else None,
        )
    ...
    async def get_health(self) -> dict[str, Any]:
        ...
        # Check LLM/compute health from coalescer stats
        coalescer_stats = stats.get("coalescer", {})
        total_computes = coalescer_stats.get("total_computes", 0)
        if total_computes > 0:
            errors = coalescer_stats.get("compute_errors", 0)
            timeouts = coalescer_stats.get("compute_timeouts", 0)
            error_rate = (errors + timeouts) / total_computes
            if error_rate >= 0.5:
