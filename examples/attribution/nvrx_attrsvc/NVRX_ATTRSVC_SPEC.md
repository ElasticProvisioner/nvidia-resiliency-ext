...
│   ├── __init__.py          # Exports ResultPoster, Slack functions
│   ├── base.py              # ResultPoster, DataflowStats (generic framework)
...
HTTP API service in examples/attribution/nvrx_attrsvc/.
...
PYTHON API REFERENCE:
    ...
    class AnalyzerConfig:
        ...
        cluster_name: str = ""               # For dataflow records
        dataflow_index: str = ""             # Elasticsearch index
...
Files (see section 2 PROJECT STRUCTURE):
    ...
    - nvrx_attrsvc/postprocessing.py          # Configures library with dataflow.post + Slack
    - nvidia_resiliency_ext/attribution/postprocessing/  # Generic framework (ResultPoster, Slack)

Configuration:
    - CLUSTER_NAME: Cluster identifier for records (env: NVRX_ATTRSVC_CLUSTER_NAME)
    - DATAFLOW_INDEX: Elasticsearch index name (env: NVRX_ATTRSVC_DATAFLOW_INDEX)
    - If DATAFLOW_INDEX is empty, dataflow posting is disabled
    - SLACK_BOT_TOKEN: Slack bot OAuth token (env: SLACK_BOT_TOKEN)
    - SLACK_CHANNEL: Slack channel for alerts (env: SLACK_CHANNEL)
...
To replace with custom implementation:
    ...
    2. Update postprocessing.py to import from new module
    3. Or replace post_results() call in service.py
...
See examples/attribution/pyproject.toml for the authoritative dependency list.
...
...........
    ...
    See examples/attribution/nvrx_attrsvc/deploy/Dockerfile
...
...............
    ...
    See examples/attribution/nvrx_attrsvc/deploy/kubernetes.yaml
