...
class AttrsvcClient:
    ...
    Handles POST/GET requests with retry logic, rate limiting, and stats fetching.
    """
    ...
    def get_stats_raw(self, pretty: bool = False) -> str | None:
        """
        Fetch raw JSON stats from the attribution service.

        Args:
            pretty: If True, request indented JSON
    ...
    def get_stats(self) -> dict[str, Any] | None:
        ...
        Returns:
            Parsed stats dict or None on failure
        """
        raw = self.get_stats_raw()
        if raw:
            try:
    ...
    def check_health_cached(self) -> bool:
        ...
        Uses instance-level cache (not global) for proper isolation in tests.
        ...
        is_healthy = self.get_stats_raw(pretty=False) is not None
    ...
    def request_with_retry(
        ...
        Args:
            method: "POST" or "GET"
            job_id: Job ID for logging (also sent to attrsvc for folder mode)
            ...
            cycle: Cycle number for GET requests in folder mode (None = latest)
        """
        ...
        for attempt in range(self._max_attempts):
            try:
                if method == "POST":
                    # Include job_id for folder mode detection
                    response = self._client.post(
                        url,
                        json={"log_path": log_path, "user": user, "job_id": job_id},
