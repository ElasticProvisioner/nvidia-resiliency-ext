...
class SlurmJobMonitor:
    ...
    def _start_status_server(self) -> None:
        ...
        self._status_server = StatusServer(
            ...
            get_attrsvc_stats_raw=self._get_attrsvc_stats_raw,
        )
    ...
    def _get_attrsvc_stats_raw(self, pretty: bool = False) -> str | None:
        """Fetch raw JSON stats from the attribution service."""
        if self._attrsvc_client is None:
            return None
        return self._attrsvc_client.get_stats_raw(pretty)

    def _get_attrsvc_stats(self) -> dict[str, Any] | None:
        """Fetch stats from the attribution service (parsed JSON)."""
        if self._attrsvc_client is None:
            return None
        return self._attrsvc_client.get_stats()

    def _get_stats_summary(self) -> str:
        """Get a one-line summary of current stats."""
        stats = self._get_stats_for_http()
        attrsvc_stats = self._get_attrsvc_stats()
        return format_stats_summary(stats, attrsvc_stats)
    ...
    def _sync_jobs_from_slurm(
        ...
        for job_id, job in current_jobs.items():
            ...
            if prev_job is None:
                ...
                if job.stdout_path:
                    self.state.total_with_output_path += 1
            elif prev_job.state != job.state:
                # State changed - preserve tracking fields
                logger.debug(
                    f"[{job_id}] State change: {prev_job.state.value} -> {job.state.value}"
                )
                job.log_submitted = prev_job.log_submitted
                job.post_success = prev_job.post_success
                job.result_fetched = prev_job.result_fetched
                job.path_fetch_attempted = prev_job.path_fetch_attempted
                job.last_state = prev_job.state
