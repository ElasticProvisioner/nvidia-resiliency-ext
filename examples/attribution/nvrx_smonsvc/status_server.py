...
def create_status_handler(
    ...
    get_attrsvc_stats_raw: Callable[[bool], str | None],
) -> type:
    """
    ...
    Args:
        ...
        get_attrsvc_stats_raw: Callback to get raw attrsvc JSON stats
    ...
    class StatusHandler(BaseHTTPRequestHandler):
        ...
        def _handle_stats(self, pretty: bool) -> None:
            """Return monitor statistics summary."""
            smonsvc_stats = get_stats()

            # Fetch attrsvc stats and embed in response
            attrsvc_raw = get_attrsvc_stats_raw(False)  # No indent needed, we'll re-serialize
            if attrsvc_raw:
                try:
                    smonsvc_stats["attrsvc"] = json.loads(attrsvc_raw)
                except json.JSONDecodeError:
                    smonsvc_stats["attrsvc"] = {"error": "invalid json"}
            else:
                smonsvc_stats["attrsvc"] = {"error": "unavailable"}

            self._send_json(smonsvc_stats, pretty=pretty)
...
class StatusServer:
    ...
    def __init__(
        ...
        self._get_attrsvc_stats_raw = get_attrsvc_stats_raw
