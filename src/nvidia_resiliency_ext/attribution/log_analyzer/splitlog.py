...
# Pattern to extract cycle number from filename: *_cycle<N>.log
CYCLE_NUM_PATTERN = re.compile(r"_cycle(\d+)\.log$", re.IGNORECASE)

# Pattern to extract date/time from filename: *_date_YY-MM-DD_time_HH-MM-SS.log
# Example: 55B_hybrid_moe_25T_phase1_1609449_date_26-01-27_time_06-20-06.log
...
class SplitlogTracker:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            "jobs": len(jobs),
            "jobs_terminated": sum(1 for j in jobs if j.terminated),
            "jobs_cleaned": self._jobs_cleaned,
            "files": {
                "tracked": sum(len(j.file_info) for j in jobs),
                "analyzed": sum(j.files_complete() for j in jobs),
            },
            "sched_restarts": sum(j.sched_restarts for j in jobs),
        }
    ...
    def _find_log_files(self, logs_dir: Optional[str], job_id: Optional[str]) -> List[str]:
        ...
        # Build pattern with job_id
        pattern = self._log_pattern.replace("{job_id}", job_id)
