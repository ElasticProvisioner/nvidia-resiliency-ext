...
from .coalescer import CacheResult, InflightResult, RequestCoalescer, SubmittedResult
from .config import DEFAULT_COMPUTE_TIMEOUT_SECONDS, TTL_PENDING_SECONDS, ErrorCode
...
class AnalyzerConfig:
    ...
    Attributes:
        ...
        cluster_name: Cluster name for dataflow records (optional)
        dataflow_index: Elasticsearch index for posting results (optional)
    """
...
class SubmitResult:
    ...
    submitted: bool
    normalized_path: str
    mode: str = "single"  # "pending", "single", or "splitlog"
...
class LogAnalyzer:
    ...
    def __init__(
        ...
        post_results_fn: Optional[Callable] = None,
    ):
        """
        ...
        Args:
            ...
            post_results_fn: Optional callback for posting results to dataflow
        """
        self.config = config
        ...
        self._post_results_fn = post_results_fn
    ...
    async def analyze(
        ...
        # Handle pending jobs
        if mode == JobMode.PENDING and job:
        ...
        try:
            ...
            return AnalysisResult(result=log_result, status="completed")
    ...
    async def _run_llm_analysis(
        ...
        async with client:
            ...
            # Post to dataflow if configured
            if (
                self._post_results_fn
                and self.config.dataflow_index
                and "result" in log_result
                and len(log_result["result"]) > 0
            ):
                for item in log_result["result"]:
                    raw_text = item[0]
                    parsed = parse_llm_response(raw_text)
                    # Use provided job_id first, fall back to path extraction only if not provided
                    if job_id:
                        # Extract cycle_id from path, use provided job_id (suppress job_id warning)
                        ...
                        # No job_id provided, extract everything from path (warn if job_id not found)
    ...
    def get_all_jobs(self) -> Dict[str, Any]:
        ...
        for job in jobs_snapshot:
            job_info = {
                ...
                "log_path": job.path,
                "user": job.user,
                "mode": job.mode.value if isinstance(job.mode, JobMode) else job.mode,
            }
            if job.is_pending():
                ...
                job_info.update(
                    {
                        ...
                        "log_files": job.known_log_files,
                    }
                )
                splitlog_jobs.append(job_info)

        return {
            "pending": {"count": len(pending_jobs), "jobs": pending_jobs},
            "single": {"count": len(single_jobs), "jobs": single_jobs},
            "splitlog": {"count": len(splitlog_jobs), "jobs": splitlog_jobs},
        }
