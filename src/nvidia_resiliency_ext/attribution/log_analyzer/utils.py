...
# Regex patterns for log file path parsing
CYCLE_LOG_PATTERN = r"_cycle(\d+)\.log$"  # Per-cycle log files (e.g., foo_cycle3.log)
...
def extract_job_metadata(log_path: str, warn_on_missing_job_id: bool = True) -> JobMetadata:
    ...
    Expected path patterns for job ID (tried in order):
    - _<job_id>_date_... (e.g., foo_12345_date_2024...)
    - job_<job_id>/... (e.g., job_12345/slurm.log)
    - slurm-<job_id>.out|.err|.log (e.g., slurm-12345.out)
    - /<job_id>.out|.err|.log (e.g., /12345678.out)
    - _<job_id>.out|.err|.log (e.g., prefix_12345678.out)

    Expected path patterns for cycle ID:
    - ..._cycle<N>.log (e.g., foo_cycle3.log)
