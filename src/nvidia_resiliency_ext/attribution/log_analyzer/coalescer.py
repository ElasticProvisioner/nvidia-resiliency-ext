...
class CacheEntry:
    ...
    file_mtime: float | None = None  # File mtime at cache time (for validation)
    file_size: int | None = None  # File size at cache time (for validation)
...
class RequestCoalescer:
            ...
            if key in self._cache:
                ...
                if cache_age < self._grace_period:
                    ...
                    if cache_entry.file_mtime is not None and cache_entry.file_size is not None:
                        ...
                        # No file metadata (e.g., timeout result) - treat as hit
                        self._stats.cache_hits += 1
                        self._submitted.pop(key, None)
                        cached_result = cache_entry.result
    ...
    async def _do_compute(
        ...
        On timeout, caches a timeout result so subsequent requests get cache hits
        instead of triggering new computes.
        ...
        try:
            ...
            # Don't store file metadata for timeout results - they should expire by TTL
            async with self._lock:
                self._stats.compute_timeouts += 1
                self._in_flight.pop(key, None)
                self._cache[key] = CacheEntry(result=timeout_result, cached_at=time.monotonic())
